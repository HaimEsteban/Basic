...
def _get_body(msg: Message) -> str:
    """Return message body as UTF-8 text (prefers text/plain, falls back to stripped HTML)."""
    # 1) any text/plain part?
    if msg.is_multipart():
    ...
    # 2) otherwise first text/html part → strip tags
    for part in (msg.walk() if msg.is_multipart() else [msg]):
        if part.get_content_type() == "text/html":
            ...
            if payload:
                import re, html
                charset = part.get_content_charset() or "utf-8"
                html_text = payload.decode(charset, "ignore")
